                ...
                for b in np.arange(nb_axis):
                    for h in np.arange(new_h):
                        for w in np.arange(new_w):
                            h_shift = h*pool_h
                            w_shift = w*pool_w
                            layer_grads[a,b,h_shift:h_shift+pool_h,w_shift:w_shift+pool_w]=pre_grad[a,b,h,w]/length
            ...
            for a in np.arange(nb_batch):
                for h in np.arange(new_h):
                    for w in np.arange(new_w):
                        h_shift = h*pool_h
                        w_shift = w*pool_w
                        layer_grads[a,h_shift:h_shift+pool_h,w_shift:w_shift+pool_w]=pre_grad[a,h,w]/length
